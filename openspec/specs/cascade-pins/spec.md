# cascade-pins Specification

## Purpose

The `cascade-pins` capability provides a Python-backed CLI tool that walks a
nested git-submodule graph from a configured root, plans and executes
deepest-first pin bumps so that each parent's submodule pointers reference
their children's latest remote-branch heads, optionally re-resolves
`uv.lock` for parents that ship a `pyproject.toml`, and emits templated
commit messages summarising the bumped children. The capability also
exposes drift detection across same-name submodules and a status view
distinct from existing `just status` recipes (pinned vs. remote-branch
SHAs, not pinned vs. local SHAs).
